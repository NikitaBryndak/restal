...
export default function TripCard({ data }: { data: Trip }) {
  ...
  const docs =
    (data.documents as Record<string, { uploaded?: boolean; url?: string }>) ||
    {};

  const uploadedMainDocs = mainDocKeys.filter(
    (key) => docs[key]?.uploaded,
  ).length;
  const uploadedTickets = ticketKeys.filter(
    (key) => docs[key]?.uploaded,
  ).length;
    ...
    <div className={rootClass}>
      ...
      <div className="relative md:absolute md:inset-0 flex flex-col md:flex-row md:items-stretch">
        ...
        <div className="shrink-0 md:flex-[0_0_30%] flex flex-col justify-center items-center p-4 sm:p-6">
          ...
          <div
            className={cn(
              "px-3 py-1 rounded-full text-xs font-semibold border mb-3",
              statusColors[status],
            )}
          >
        ...
        <div className="flex w-full p-3 sm:p-6 gap-3 sm:gap-5 flex-col md:flex-row">
          ...
          <div className="flex flex-row md:flex-col md:flex-[0_0_40%] gap-3">
            <div className="bg-white/10 backdrop-blur-md rounded-lg p-3 sm:p-4 flex-1 md:h-full text-white flex flex-col justify-center">
              <p className="text-[10px] uppercase tracking-[0.2em] text-white/60 text-center mb-2">
                Оплата
              </p>
              <div className="flex flex-col gap-2">
                <div className="text-center">
                  ...
                  <p className="text-[10px] sm:text-[11px] text-emerald-300 font-medium mt-1">
                    💰 +
                    {new Intl.NumberFormat("de-DE", {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                ...
                <div className="text-center">
                  <p className="text-[10px] sm:text-[11px] text-amber-300/90 leading-relaxed mt-1">
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-lg p-3 sm:p-4 text-white flex flex-col gap-2">
              <p className="text-[10px] uppercase tracking-[0.2em] text-white/60 text-center">
                Документи
              </p>
